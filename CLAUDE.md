...
- **Frontend-only changes:** `python3 publish.py` from the Dropbox project
  ...
  changes can land there before Dropbox catches up).
- **Backend (`api.py`) changes:** need `flyctl deploy` from
  `Coaches Station API/` — a separate, real deploy, not covered by
  `publish.py` at all. Go ahead and run `flyctl deploy` directly when a
  backend change is ready — no need to ask for confirmation first, that's
  pre-authorized standing instruction for this repo. Still test against a
  local copy first where possible (see "Local dev" below), and say so
  explicitly when that isn't possible (no local DB, no way to exercise the
  changed code path) rather than deploying untested changes silently.
...
- **Backend:** `api.py` reads its database path from `DB_PATH`, which is
  ...
  (built via `load_nhl.py` from a handful of sample games) works with zero
  code changes.
